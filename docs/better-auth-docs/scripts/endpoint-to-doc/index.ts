...
fs.watch(file, async () => {
    ...
    if (inputCode.includes(".coerce")) fs.writeFileSync(file, inputCode.replaceAll(".coerce", ""), "utf-8");
    await generateMDX();
    playSound("Hero");
});
...
async function generateMDX() {
    ...
    clearImportCache();
    if (Object.keys(exports).length !== 1) return console.error(`Please provide at least 1 export.`);
...
export const createAuthEndpoint = async (...params: Partial<CreateAuthEndpointProps>) => {
    ...
    for (const body of parsedBody) {
        ...
        if (body.jsDocComment || body.isServerOnly) {
            strBody += `${indentationSpaces.repeat(
                1 + body.path.length
            )}/**\n${indentationSpaces.repeat(1 + body.path.length)} * ${body.jsDocComment} ${
        ...
        if (!parsedBody[i + 1] || parsedBody[i + 1].path.length < body.path.length) {
...
function parseZodShape(zod: z.ZodAny, path: string[]) {
    ...
    let shape = z.object({ test: z.string({ description: "" }) }, { description: "some descriptiom" }).shape;
    ...
    for (const [key, value] of Object.entries(shape)) {
        ...
        let isServerOnly = description ? description.includes("server-only.") : false;
...
function getType(
    ...
    switch (value._def.typeName as string) {
        ...
        case "ZodArray": {
            ...
            return {
                type: [`${types.type.length === 1 ? types.type[0] : `(${types.type.join(" | ")})`}[]`, ..._null],
                isOptional: forceOptional ?? v.isOptional(),
                defaultValue: forceDefaultValue,
            };
        }
...
function parseParams(path: string, options: Options): string {
    ...
    if (options.requireHeaders || isUsingSessionMiddleware) params.push("requireSession");
