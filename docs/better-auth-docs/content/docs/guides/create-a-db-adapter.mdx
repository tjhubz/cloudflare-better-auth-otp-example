...
<Callout>Make sure to return the data that is inserted into the database.</Callout>
...
<Callout>Make sure to return the array of data that is found in the database.</Callout>
...
<Callout type="warn">
    If you're using `supportsJSON`, `supportsDates`, or `supportsBooleans`, then the transformations will be applied
    before your `customTransformInput` function is called.
</Callout>
