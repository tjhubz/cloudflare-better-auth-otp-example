    ...
    return (
        <>
            <div className="relative">
                <div id={`api-method${pathId}`} aria-hidden className="absolute invisible -top-[100px]" />
            </div>
            <Tabs defaultValue={isServerOnly ? "server" : "client"} className="w-full gap-0">
                ...
                <TabsContent value="client">
                    {isServerOnly ? null : (
                        <Endpoint method={method || "GET"} path={path} isServerOnly={isServerOnly ?? false} />
                    )}
                    {clientOnlyNote || note ? (
                        <Note>
...
function getChildren(
    x: ({ props: { children: string } } | string) | ({ props: { children: string } } | string)[]
): string[] {
    if (Array.isArray(x)) {
        const res = [];
        for (const item of x) {
...
function TypeTable({ props, isServer }: { props: Property[]; isServer: boolean }) {
    ...
    return (
        <Table className="mt-2 mb-0 overflow-hidden">
            ...
            <TableBody>
                {props.map((prop, i) =>
                    (prop.isServerOnly && isServer === false) || (prop.isClientOnly && isServer === true) ? null : (
                        <TableRow key={i}>
                            <TableCell>
                                <code>
                                ...
                                {prop.isServerOnly ? (
                                    <span className="mx-2 text-xs text-muted-foreground">(server-only)</span>
                                ) : null}
                            </TableCell>
...
function parseCode(children: JSX.Element) {
    ...
    const arrayOfCode: string[] = arrayOfJSXCode
        .flatMap((x: { props: { children: string } } | { props: { children: string } }[]) => {
    ...
    for (let line of arrayOfCode) {
        ...
        if (line.toLowerCase().startsWith("type") && !hasAlreadyDefinedApiMethodType && !withinApiMethodType) {
        ...
        if (line.startsWith("/*") || line.startsWith("*") || line.startsWith("*/")) {
            ...
            let propType = line.replace(propName, "").replace("?", "").replace(":", "").split("=")[0].trim()!;
            ...
            const description = currentJSDocDescription.length > 0 ? currentJSDocDescription : null;
            if (description) {
            ...
            const property: Property = {
                ...
                path: isTheStartOfNest ? nestPath.slice(0, nestPath.length - 1) : nestPath.slice(),
...
function createClientBody({ props }: { props: Property[] }) {
    ...
    for (const prop of props) {
        ...
        }${prop.type === "Object" ? ": {" : ","}${addComment ? ` // ${comment.join(", ")}` : ""}\n`;
    ...
    for (const prop of props) {
        ...
        if (
            ...
            !(
                ...
                props.find(
                    x =>
                        x.path.join(".") === prop.path.slice(0, prop.path.length - 2).join(".") &&
...
function Note({ children }: { children: ReactNode }) {
    return (
        <div className="relative flex flex-col w-full gap-2 p-3 mb-2 break-words border rounded-md text-md text-wrap border-border bg-fd-secondary/50">
            <span className="w-full -mb-1 text-xs select-none text-muted-foreground">Notes</span>
