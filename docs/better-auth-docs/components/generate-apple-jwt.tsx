...
const appleJwtSchema = z.object({
    ...
    clientId: z.string().min(1, { message: "Client ID (Service ID) is required." }),
    ...
    const onSubmit = async (data: AppleJwtFormValues) => {
        ...
        startTransition(() => {
            try {
                ...
                setError(`Failed to generate JWT: ${err.message || "Unknown error"}. Check key format and details.`);
            }
        });
    };
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormDescription>The ID associated with your private key (.p8 file).</FormDescription>
                                <FormMessage />
                            </FormItem>
                        )}
                    />
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormDescription>
                                    Paste the entire content of your .p8 private key file here. Ensure it's in PKCS#8
                                    format.
                                </FormDescription>
                                <FormMessage />
                            </FormItem>
            ...
            {generatedJwt && (
                <div className="mt-6 space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                        This is the client secret (JWT) required for 'Sign in with Apple'. It expires in 180 days.
                    </p>
                </div>
            )}
        </div>
