...
export const NavbarProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    // @ts-ignore
    return (
        <NavbarContext.Provider value={{ isOpen, toggleNavbar, isDocsOpen, toggleDocsNavbar }}>
            {children}
                    ...
                    <Fragment key={menu.name}>
                        {menu.child ? (
                            ...
                            <Link
                                ...
                                onClick={toggleNavbar}
                            >
                                {isDocs && <ChevronRight className="ml-0.5 size-4 text-muted-foreground md:hidden" />}
                                {menu.name}
                            </Link>
                        )}
                    </Fragment>
